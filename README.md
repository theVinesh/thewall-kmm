...
Host-owned interface for shown-state persistence. Both public functions are `suspend`.

| Function | Purpose |
| --- | --- |
| `suspend fun hasBeenShown(): Boolean` | Tells the wall wrapper whether to render |
| `suspend fun markAsShown()` | Records that the CTA-driven wall has been completed |
